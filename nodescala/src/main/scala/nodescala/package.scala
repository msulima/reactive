...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      Future.sequence(fs)
      /*val foo = (0 until fs.size) zip fs
      val result = ListBuffer[(Int, T)]()
      ...
      for {
        ...
        f onComplete {
          case Success(v) => {
            result += (index -> v)
            if (result.size == fs.size) {
              p.trySuccess(result.result().sortBy(_._1).map(_._2))
            }
          }
        }
      }
    ...
    def delay(t: Duration): Future[Unit] = {
      ...
      async {
        try {
          ...
          case e: TimeoutException => p.success()
        }
      }
      p.future
    }
